# MS2Purifier_model_generation
# This R script is to generate random forest model for MS2 spectra purification.
# Shipei Xing, Dec 14, 2020
...
# Parameter settings
step <- 1       # Numeric value, "1" or "2".
binary <- T     # logical, true for constructing a binary classification model, false for multi-label classification
vendor <- 1     # select the vendor index: Bruker: 1; Agilent: 2; Thermo: 3  
...
if(step == 1){
  ...
  dir.create('EIC_plots')
  EIC_directory <- paste0(file_directory,'/EIC_plots')
  ...
  if(vendor == 1){
    ...
    for(i in 1:nrow(DDA_ft)) { #for each metabolic feature, use AIF data to generate data matrix for RF
      ...
      for(l in 1:length(fragMz_v)) { # fragment EIC
        ...
        setwd(EIC_directory)
        figure_name <- paste0('preMz_',round(t_mz,4),'_fragMz_',round(f_mz,4),'_rt_',round(t_rt,2))
        png(paste0(figure_name,'.png'),width = 5000,height = 5000,res = 800)
        plot(AIF_ms1rt[AIF_rt_index],peak_smooth(AIF_pre_int,level=2),type='l',col='red',pch=20,ylim=c(0,max(max(AIF_frag_int),max(AIF_pre_int))))
        lines(AIF_ms1rt[AIF_rt_index],peak_smooth(AIF_frag_int,level=2),col='green',pch=20)
        title(figure_name)
        dev.off()
    ...
    rf_df$manual_check <- NA
    
    write.csv(rf_df, file = 'Fragment_for_manual_check.csv',row.names = F)
    
    message('Please work on the manual inspections and come back later for step 2. :)')
...
if(step == 2){
  setwd(file_directory)
  df <- read.csv('Fragment_for_manual_check.csv',stringsAsFactors = F)[,5:11]
  df <- df[complete.cases(df),]
  df$manual_check <- as.factor(df$manual_check)
  
  if(binary){
    class_v <- c('T'=nrow(df)/as.numeric(table(df$manual_check)[names(table(df$manual_check))=='T']),
                 'F'=nrow(df)/as.numeric(table(df$manual_check)[names(table(df$manual_check))=='F']))
    
    # generate RF model for 10 times, choose the best one (with least OOB error rate)
    oob_v <- vector()
    for(i in 1:10){
      set.seed(i)
      
      rf.model <- randomForest(manual_check~.,data=df,
                               classwt = class_v,ntree=1000)
      oob_v[i] <- rf.model$err.rate[1000,1]
    }
    index <- match(min(oob_v), oob_v)
    
    set.seed(index)
    rf.model <- randomForest(manual_check~.,data=df,
                             classwt = class_v,ntree=1000)
    saveRDS(rf.model, file = 'RF_new_binary_model.rds')
  }
  ...
  if(!binary){
    ...
    for(i in 1:10){
      set.seed(i)
      rf.model <- randomForest(manual_check~.,data=df,ntree=1000)
      oob_v[i] <- rf.model$err.rate[1000,1]
    }
    index <- match(min(oob_v), oob_v)
    ...
    set.seed(index)
    rf.model <- randomForest(manual_check~.,data=df,ntree=1000)
    saveRDS(rf.model, file = 'RF_new_multi_model.rds')
  }

  message("New RF models have been generated! :)")
}