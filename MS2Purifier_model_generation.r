...
if(step == 1){
  ...
  if(vendor == 1){
    ...
    for(i in 1:nrow(featureTable)){
      ...
      for(k in 1:length(fileindex)){ # go through files in order (high to low int)
...
if(step == 2){
  ...
  for(i in 1:10){
    ...
    rf.model <- randomForest(manual_check~.,data=df,
                             classwt = class_v,ntree=1000,mtry=2)
  ...
  rf.model <- randomForest(manual_check~.,data=df,
                           classwt = class_v,ntree=1000,mtry=2)
  saveRDS(rf.model, file = 'RF_new_model.rds')
  message("A new RF model has been generated! :)")
}