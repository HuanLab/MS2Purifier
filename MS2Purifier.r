...
rf_binary_model <- readRDS('RF_model_binary_20201105.rds')
rf_multi_model <- readRDS('RF_model_multi_20201105.rds')
...
if(vendor == 1){
  ...
  rf_df_pred$prediction[rf_df_pred$ideal_slope_pre==0 & rf_df_pred$sharpness_pre==0] <- as.factor('T')
  rf_df_pred$prediction[rf_df_pred$ideal_slope_frag==0 & rf_df_pred$sharpness_frag==0] <- as.factor('T')
  ...
  if(multi_classification==T){
    ...
    rf_df_pred_multi$prediction <- pred_multi
