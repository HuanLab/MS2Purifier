...
model_directory <- 'E:/MS2Purifier_2020/R_scripts&model/model'  # directory containing machine learning models
...
multi_classification <- T # logical, TRUE for conducting multi-label classification
SCI_threshold <- 1   # Threshold of high-confidence solvent contamination ions (in percentage), default 1 %.
...
if(vendor == 1){
  ...
  if(multi_classification==T){
    ...
    # solvent contamination ions
    a <- sort(table(round(rf_df_pred_multi$DDA_fragMz[rf_df_pred_multi$prediction == 'Solvent_contaminant'],2)),decreasing = T)
    threshold_no <- round((nrow(DDA_ft)* SCI_threshold / 100))
    high_SCI <- as.numeric(names(a[a >= threshold_no]))
    
    rf_df_pred_multi$prediction[rf_df_pred_multi$prediction == 'Solvent_contaminant'] <- 'Solvent_contaminant_low_confidence'
                                
    rf_df_pred_multi$prediction[rf_df_pred_multi$prediction == 'Solvent_contaminant_low_confidence' 
                                & round(rf_df_pred_multi$DDA_fragMz,2) %in% high_SCI] <- 'Solvent_contaminant_high_confidence'
  ...
  for(i in 1:length(unique(rf_df_pred$featureID))){
    ...
    p_frag_v <- rf_df_pred$DDA_fragMz[rf_df_pred$featureID == id & rf_df_pred$prediction=='T']
    ...
    p_fragint <- paste0(rf_df_pred$DDA_fragint[rf_df_pred$featureID == id & 
                                                 rf_df_pred$prediction=='T'], collapse = ';')
